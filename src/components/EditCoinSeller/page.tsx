...
/**
 * Renders the EditCoinSeller component, which allows the user to edit the details of a coin seller.
 *
 * @param props - An object containing the following properties:
 *   - `formData`: The initial form data for the coin seller.
 *   - `fetchData`: A function to fetch the updated data after the edit is successful.
 *   - `setOpenEditManagerModal`: A function to set the state of the edit manager modal.
 * @returns The rendered EditCoinSeller component.
 */
export default function EditCoinSeller(props: EditCoinSellerProps) {
  const [editFormData, setEditFormData] =
    useState<EditFormData>(initialFormState);
  ...
  const handleEdit = async () => {
    try {
      ...
      formData.set("seller_name", editFormData?.username);
      formData.set("userId", editFormData?.userId);
      formData.set("password", editFormData?.password);
      ...
      <div className="w-full">
        ...
        <Select
          ...
          onChange={(selectedOption) => {
            setEditFormData((prevState) => ({
              ...prevState,
              selectedCountry: selectedOption as {
                label: string;
                value: string;
              },
            }));
